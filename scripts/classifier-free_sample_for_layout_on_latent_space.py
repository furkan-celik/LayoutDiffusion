...
def main():
    ...
    parser.add_argument(
        "--config_file", type=str, default="./configs/LayoutDiffusion-v1.yaml"
    )
    ...
    logger.log(
        "current rank == {}, total_num = {}, \n, {}".format(
            dist.get_rank(), dist.get_world_size(), cfg
        )
    )
    ...
    if cfg.sample.pretrained_model_path:
        ...
        checkpoint = dist_util.load_state_dict(
            cfg.sample.pretrained_model_path, map_location="cpu"
        )
    ...
    def model_fn(
        x, t, obj_class=None, obj_bbox=None, obj_mask=None, is_valid_obj=None, **kwargs
    ):
        ...
        obj_class = th.ones_like(obj_class).fill_(
            model.layout_encoder.num_classes_for_layout_object - 1
        )
        obj_class[:, 0] = 0
    ...
    if cfg.sample.save_images_with_bboxs:
        dir_names.extend(
            [
                "real_imgs_with_bboxs",
                "generated_imgs_with_bboxs",
                "generated_images_with_each_bbox",
            ]
        )
    if cfg.sample.save_sequence_of_obj_imgs:
        dir_names.extend(
            ["obj_imgs_from_unresized_gt_imgs", "obj_imgs_from_resized_gt_imgs"]
        )
        ...
        if cfg.data.type == "COCO-stuff":
            for class_id in range(1, 183):  # 1-182
                if class_id not in [12, 183, 26, 29, 30, 45, 66, 68, 69, 71, 83, 91]:
                    os.makedirs(
                        os.path.join(log_dir, "generated_cropped_imgs", str(class_id)),
                        exist_ok=True,
                    )
                    os.makedirs(
                        os.path.join(log_dir, "real_cropped_imgs", str(class_id)),
                        exist_ok=True,
                    )
    ...
    if cfg.sample.sample_method == "dpm_solver":
        ...
        diffusion = build_diffusion(
            cfg, timestep_respacing=cfg.sample.timestep_respacing
        )
    else:
        raise NotImplementedError
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        for sample_idx in range(cfg.sample.sample_times):
            ...
            if cfg.sample.sample_method == "dpm_solver":
                ...
                x_T = th.randn(
                    (
                        imgs.shape[0],
                        cfg.model.parameters.in_channels,
                        cfg.model.parameters.image_size,
                        cfg.model.parameters.image_size,
                    ),
                ...
                sample_fn = (
                    ...
                    if cfg.sample.sample_method == "ddpm"
            ...
            for img_idx in range(imgs.shape[0]):
                ...
                if cfg.sample.save_cropped_images:
                    obj_imgs_from_resized_generated_imgs = get_cropped_image(
                        obj_bboxes=obj_bbox[1 : 1 + obj_num].unsqueeze(
                            0
                        ),  # (1, L, 4), N=1
                    ...
                    for obj_idx in range(obj_num):
                        saved_cropped_image_name = "{}_{}_{}_{}_{}.png".format(
                            filename,
                            obj_idx,
                            obj_class[1 : 1 + obj_num][obj_idx],
                            obj_name[1 : 1 + obj_num][obj_idx].replace(" ", "-"),
                            sample_idx,
                        )
                        utils.save_image(
                            image_unnormalize_batch(
                                obj_imgs_from_resized_generated_imgs.squeeze(0)[
                                    obj_idx : obj_idx + 1
                                ]
                            ),
                            os.path.join(
                                log_dir,
                                "generated_cropped_imgs/{}/{}".format(
                                    obj_class[1 : 1 + obj_num][obj_idx],
                                    saved_cropped_image_name,
                                ),
                            ),
                            nrow=1,
                        )
                ...
                if cfg.sample.save_images_with_bboxs:
                    ...
                    for i, is_valid in enumerate(is_valid_obj):
                        if not is_valid:
                            generated_images_with_each_bbox.append(
                                (sample[img_idx] * 127.5 + 127.5).to(torch.uint8).cpu()
                            )
                            real_images_with_each_bbox.append(
                                (imgs[img_idx] * 127.5 + 127.5).to(torch.uint8).cpu()
                            )
                        else:
                            generated_images_with_each_bbox.append(
                                utils.draw_bounding_boxes(
                                    image=(sample[img_idx] * 127.5 + 127.5)
                                    .to(torch.uint8)
                                    .cpu(),
                            ...
                            real_images_with_each_bbox.append(
                                utils.draw_bounding_boxes(
                                    image=(imgs[img_idx] * 127.5 + 127.5)
                                    .to(torch.uint8)
                                    .cpu(),
                                    boxes=absolute_obj_bbox[i : i + 1],
                                    labels=[cond["obj_class_name"][img_idx][i]],
                    ...
                    generated_images_with_each_bbox = (
                        torch.stack(generated_images_with_each_bbox, dim=0).cpu()
                        / 255.0
                    )  # (layout_length, 3, h, w)
                    real_images_with_each_bbox = (
                        torch.stack(real_images_with_each_bbox, dim=0).cpu() / 255.0
                    )  # (layout_length, 3, h, w)
                    utils.save_image(
                        torch.cat(
                            [
                ...
                if sample_idx == 0:
                    ...
                    if cfg.sample.save_cropped_images:
                        ...
                        for obj_idx in range(obj_num):
                            ...
                            utils.save_image(
                                image_unnormalize_batch(
                                    obj_imgs_from_resized_real_imgs.squeeze(0)[
                                        obj_idx : obj_idx + 1
                                    ]
                                ),
                                os.path.join(
                                    log_dir,
                                    "real_cropped_imgs/{}/{}".format(
                                        obj_class[1 : 1 + obj_num][obj_idx],
                                        saved_cropped_image_name,
                                    ),
                                ),
                    ...
                    if cfg.sample.save_images_with_bboxs:
                        ...
                        imageio_save_image(
                            ...
                            path=os.path.join(
                                log_dir, "real_imgs_with_bboxs/{}.png".format(filename)
                            ),
                        )
                    ...
                    if cfg.sample.save_sequence_of_obj_imgs:
                        ...
                        if "obj_image" in list(cond.keys()):
                            obj_imgs_from_unresized_gt_imgs = cond["obj_image"][
                                img_idx
                            ]  # (L, 3, M, M)
                            utils.save_image(
                                image_unnormalize_batch(
                                    obj_imgs_from_unresized_gt_imgs
                                ),
                                os.path.join(
                                    log_dir,
                                    "obj_imgs_from_unresized_gt_imgs/{}.png".format(
                                        filename
                                    ),
                                ),
                                nrow=cfg.data.parameters.layout_length,
                        ...
                        utils.save_image(
                            image_unnormalize_batch(
                                obj_imgs_from_resized_gt_imgs.squeeze(0)
                            ),
                            os.path.join(
                                log_dir,
                                "obj_imgs_from_resized_gt_imgs/{}.png".format(filename),
                            ),
                            nrow=cfg.data.parameters.layout_length,
                        )
        ...
        cur_num_samples = (
            (batch_idx + 1)
            * cfg.data.parameters.test.batch_size
            * dist.get_world_size()
        )
        fps = (
            (batch_idx + 1)
            * cfg.data.parameters.test.batch_size
            * cfg.sample.sample_times
            / (total_time)
        )
        logger.log(
            "FPS = {} / {} = {} imgs / second".format(
                (batch_idx + 1)
                * cfg.data.parameters.test.batch_size
                * cfg.sample.sample_times,
                total_time,
        ...
        logger.log(
            f"batch_id={batch_idx + 1} created {cur_num_samples} / {total_num_samples} samples"
        )
