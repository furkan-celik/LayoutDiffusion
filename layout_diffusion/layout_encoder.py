...
class LayoutTransformerEncoder(nn.Module):
    def __init__(
        ...
        if "obj_class" in self.used_condition_types:
            ...
            self.obj_class_embedding = nn.Linear(
        ...
        for resolution in self.resolution_to_attention:
            ...
            self.image_patch_bbox_embedding["resolution{}".format(resolution)] = (
                torch.FloatTensor(
                    [
                        (
                            interval * j,
    ...
    def forward(
        ...
        assert (
            (obj_class is not None) or (obj_bbox is not None) or (obj_mask is not None)
        )
        outputs = {}
        ...
        if "obj_class" in self.used_condition_types:
            obj_class_embedding = self.obj_class_embedding(obj_class.to(self.dtype))
            if xf_in is None:
